...
rcfg.add_stage2_to_sys_path()
import stage2_cf_prescription_generation as stage2_prescriptions  # noqa: E402


# Path resolution delegated to reporter_config
...
def _load_current_levels(*, stage2_root: Path) -> pd.DataFrame:
    ...
    if not analysis_path.exists():
        analysis_path = rcfg.ANALYSIS_DATASET_CSV
    if not analysis_path.exists():
    ...
    usecols = ["Location ID"] + list(rcfg.ALL_TREATMENTS)
    ...
    rename_map = {t: stage2_prescriptions.CURRENT_LEVEL_COLUMN_MAP[t] for t in rcfg.ALL_TREATMENTS if t in df.columns}
...
def compute_scenario_metrics(df: pd.DataFrame, scenario: Scenario, population: str) -> dict:
    ...
    eps_y = float(getattr(rcfg, "OUTCOME_EPSILON", 0.001))
    ...
    for treatment in stage2_prescriptions.TREATMENT_CF_SLUG.keys():
...
def main() -> int:
    ...
    stage2_root = rcfg.resolve_stage2_root(stage2_output_dir=args.stage2_output_dir, run_id=args.run_id)
    ...
    # Outputs
    reports_dir: Path = rcfg.ensure_reports_dir(stage2_root)
