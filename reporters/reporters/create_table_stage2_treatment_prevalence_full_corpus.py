...
CODEBOOK_CSV = rcfg.CODEBOOK_CSV
...
def load_codebook_label_map(codebook_csv: Path) -> dict[str, dict[int, str]]:
    ...
    wanted = set(rcfg.ALL_TREATMENTS)
...
def build_table(segments_unique_csv: Path) -> pd.DataFrame:
    ...
    for treatment in rcfg.ALL_TREATMENTS:
    ...
    out["Treatment"] = pd.Categorical(out["Treatment"], categories=rcfg.ALL_TREATMENTS, ordered=True)
...
def main() -> None:
    segments_csv = rcfg.SEGMENTS_UNIQUE_CSV
    reports_dir = rcfg.ensure_reports_dir()
