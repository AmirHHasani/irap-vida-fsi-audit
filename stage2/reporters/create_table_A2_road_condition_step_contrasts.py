...
def _baseline_fsi_natural(df: pd.DataFrame) -> np.ndarray:
    ...
    """
    eps_y = float(getattr(config, "OUTCOME_EPSILON", 1e-4))

    if hasattr(config, "TARGET_COL") and config.TARGET_COL in df.columns:
        baseline = pd.to_numeric(df[config.TARGET_COL], errors="coerce").astype(float).values
...
def _build_features(analysis_data: pd.DataFrame) -> pd.DataFrame:
    ...
    available_controls = [
        col for col in config.CONTROL_FEATURES
...
def load_merged_data() -> pd.DataFrame:
    ...
    stage1_hotspots = pd.read_csv(config.STAGE1_HOTSPOT_OVERLAY)
    stage1_predictions = pd.read_csv(config.STAGE1_OOF_PREDICTIONS, low_memory=False)
    original_data = pd.read_csv(config.SEGMENTS_DATA_CSV, low_memory=False)
