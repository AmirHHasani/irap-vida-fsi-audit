...
def main() -> None:
    ...
    stage2_dir = _resolve_stage2_output_dir(stage2_output_dir=args.stage2_output_dir, run_id=args.run_id)
    hotspot_csv = stage2_dir / "hierarchical_cf" / "hotspot_level" / "hotspot_segments_detailed.csv"

    segments_csv = Path(args.segments_unique_csv).expanduser().resolve() if args.segments_unique_csv else config.SEGMENTS_DATA_CSV

    if args.out_csv:
        out_csv = Path(args.out_csv).expanduser().resolve()
        out_csv.parent.mkdir(parents=True, exist_ok=True)
    else:
        out_dir = stage2_dir / "reports"
