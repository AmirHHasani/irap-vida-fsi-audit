...
def resolve_inputs(*, stage2_root: Path, irap_countermeasures_csv: Path | None = None) -> Inputs:
    ...
    # iRAP file path: keep existing convention used elsewhere in repo unless overridden.
    if irap_countermeasures_csv is None:
        irap_countermeasures_csv = (
            Path(__file__).resolve().parent
            / ".."
            / "all_data"
            / "666902 Countermeasures - Excluding Overridden.csv"
        ).resolve()
...
def build_metrics_output(
    ...
    both_silent = [seg for seg in candidate_ids if seg not in p5_any and seg not in irap_any]
...
if __name__ == "__main__":
    ...
    if set(config.ALL_TREATMENTS) != set(V27_TREATMENTS):
